# 01_run_split_challenging_comparison.R
#
# Compare the current CP + Cox upper-tau method against a half-half split
...
parse_cli <- function(args) {
  ...
  for (a in args) {
    ...
    if (key %in% c("test_frac", "beta_scale", "sigma", "cens_target", "alpha", "split_frac")) {
...
run_one_rep_both_methods <- function(n,
  ...
  split_fit <- conformal_pi_cox_Tstar_split_support_fit(
    train_df = train_df,
    test_df = test_df,
    tau = tau,
    alpha = alpha,
  ...
  split_eval <- evaluate_interval_fit(
    fit_obj = split_fit,
    method_label = "CP + Cox (Half-Split)",
    method_id = "cp_cox_halfsplit"
  )
  ...
  list(
    ...
    method_rows = bind_rows(
      ...
      tibble(
        method_id = "cp_cox_halfsplit",
        method_label = "CP + Cox (Half-Split)",
        coverage = split_eval$coverage,
        mean_len = split_eval$mean_len,
        sd_len = split_eval$sd_len
      )
    )
  )
}
...
summarise_by_method <- function(rep_df, alpha) {
  ...
  rep_df %>%
    group_by(
      ...
      n, test_frac, p, beta_scale, sigma, cens_target, alpha, B, split_frac
    ) %>%
    summarise(
      n_rep = n(),
...
summarise_paired_differences <- function(rep_df) {
  rep_df %>%
    ...
    mutate(
      coverage_diff_halfsplit_minus_full = coverage_cp_cox_halfsplit - coverage_cp_cox_full,
      length_diff_halfsplit_minus_full = mean_len_cp_cox_halfsplit - mean_len_cp_cox_full
    ) %>%
    group_by(scenario_id, scenario_label, tau_quantile) %>%
    summarise(
      n_rep = n(),
      coverage_diff_mean = mean(coverage_diff_halfsplit_minus_full, na.rm = TRUE),
      length_diff_mean = mean(length_diff_halfsplit_minus_full, na.rm = TRUE),
      .groups = "drop"
    ) %>%
...
main <- function() {
  ...
  source(file.path(project_root, "playground", "split-support-fit-comparison", "00_split_support_fit_methods.R"))
  ...
  if (is.null(out_dir) || !nzchar(out_dir)) {
    out_dir <- file.path(project_root, "playground", "split-challenging-dgp-comparison", "results", cfg$tag)
  } else if (!is_absolute_path(out_dir)) {
  ...
  scenario_ids <- intersect(cfg$scenario_ids, catalog$scenario_id)
  if (!length(scenario_ids)) stop("No valid scenario_ids supplied.")

  rep_rows <- vector("list", length = length(scenario_ids) * length(cfg$tau_quantiles) * cfg$n_rep)
  idx_row <- 0L
  ...
  for (scenario_id in scenario_ids) {
    ...
    for (tau_q in cfg$tau_quantiles) {
      for (rep in seq_len(cfg$n_rep)) {
        seed <- cfg$seed0 + 100000L * match(scenario_id, scenario_ids) + 1000L * round(100 * tau_q) + rep
        ...
        idx_row <- idx_row + 1L
        rep_rows[[idx_row]] <- out$method_rows %>%
          mutate(
            scenario_id = scenario_id,
  ...
  overall_summary <- summarise_overall(rep_df, alpha = cfg$alpha)
  setting_method_summary <- summarise_by_method(rep_df, alpha = cfg$alpha)
  paired_diffs <- summarise_paired_differences(rep_df)
  ...
  write_csv(setting_method_summary, file.path(derived_dir, "setting_method_summary.csv"))
  write_csv(paired_diffs, file.path(derived_dir, "paired_setting_differences.csv"))

# 01_run_split_support_fit_comparison.R
#
# Compare the final main CP + Cox upper-tau method against an exploratory
# split-training variant that uses one training half for support estimation
# and the other training half for Cox working-model fitting.
...
parse_cli <- function(args) {
  cfg <- list(
    tag = "main-grid-split-support-fit-v1",
    out_dir = NULL,
    n_rep = 30L,
    n = 2000L,
    test_frac = 0.30,
  ...
  for (a in args) {
    ...
    if (key %in% c("n_rep", "n", "p", "high_dim_p", "B", "seed0", "sample_n")) {
      cfg[[key]] <- as.integer(val)
    }
    if (key %in% c("test_frac", "alpha", "split_frac")) {
...
evaluate_interval_fit <- function(fit_obj, method_label, method_id, test_df) {
...
run_one_rep_both_methods <- function(n,
  ...
  split_eval <- evaluate_interval_fit(
    fit_obj = split_fit,
    method_label = "CP + Cox (Split Support/Fit)",
    method_id = "cp_cox_split_support_fit",
    test_df = test_df
  )
  ...
  list(
    ...
    test_n = nrow(test_df),
    split_support_n = split_fit$support_n,
    split_fit_n = split_fit$fit_n,
    ...
    sample_rows = bind_rows(current_eval$res, split_eval$res)
  )
}
...
summarise_paired_differences <- function(rep_df) {
  rep_df %>%
    ...
    mutate(
      coverage_diff_split_minus_full = coverage_cp_cox_split_support_fit - coverage_cp_cox_full,
      length_diff_split_minus_full = mean_len_cp_cox_split_support_fit - mean_len_cp_cox_full
    ) %>%
    group_by(setting_id, dgp_id, dgp_label, cens_target, tau_quantile) %>%
    summarise(
      n_rep = n(),
      coverage_diff_mean = mean(coverage_diff_split_minus_full, na.rm = TRUE),
      coverage_diff_sd = sd(coverage_diff_split_minus_full, na.rm = TRUE),
      length_diff_mean = mean(length_diff_split_minus_full, na.rm = TRUE),
      length_diff_sd = sd(length_diff_split_minus_full, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    ...
    for (r in seq_len(cfg$n_rep)) {
      ...
      rep_rows[[ptr]] <- out$method_rows %>%
        mutate(
          ...
          train_n = out$train_n,
          test_n = out$test_n,
          split_support_n = out$split_support_n,
          split_fit_n = out$split_fit_n
        )
...
write_outputs <- function(scenarios, results, cfg, out_dir) {
  ...
  run_config <- tibble(
    tag = cfg$tag,
    out_dir = out_dir,
    methods = "CP + Cox; CP + Cox (Split Support/Fit)",
    split_frac = cfg$split_frac,
    n_rep = cfg$n_rep,
    n = cfg$n,
    test_frac = cfg$test_frac,
...
main <- function() {
  ...
  source(file.path(script_dir, "00_split_support_fit_methods.R"))
  ...
  if (is.null(cfg$out_dir) || identical(cfg$out_dir, "")) {
    out_dir <- file.path(project_root, "playground", "split-support-fit-comparison", "results", cfg$tag)

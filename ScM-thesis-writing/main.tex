...
\definecolor{reviewpurple}{RGB}{128,0,96}
\definecolor{revisionblue}{RGB}{0,76,122}
...
\textcolor{reviewpurple}{%
Acknowledgements to be finalized.
}
...
\begin{center}
\textcolor{reviewpurple}{\textit{Dedication to be finalized.}}
\end{center}
...
Conformal prediction (CP) provides finite-sample uncertainty quantification under exchangeability by converting model outputs into calibrated prediction sets.
Its central advantage is a separation between \emph{modeling} and \emph{calibration}: a learning algorithm can be used as a working score generator, while CP enforces a marginal error guarantee.

Following \citet{vovk2022algorithmic}, let $z_i=(x_i,y_i)$ be exchangeable examples.
Given a nonconformity measure $A$, for candidate label $y$ of a new object $x_n$, define
\[
\alpha_i(y)=A\!\left(\{z_1,\ldots,z_{n-1},(x_n,y)\},z_i\right),\quad i=1,\ldots,n,
\]
where $z_n=(x_n,y)$.
The full conformal p-value is
\[
p_n(y)=\frac{1}{n}\left|\left\{i\in\{1,\ldots,n\}:\alpha_i(y)\ge \alpha_n(y)\right\}\right|,
\]
and the prediction set at miscoverage level $\epsilon$ is
\[
\Gamma_\epsilon(x_n)=\{y:\; p_n(y)>\epsilon\}.
\]
The conformal p-value defined above is conservative when ties occur among
the conformity scores. A common modification is the smoothed conformal
predictor, which resolves ties by randomization.

Specifically, the smoothed conformal p-value is defined as

\[
p_n^{\text{sm}}(y)
=
\frac{
|\{i:\alpha_i(y)>\alpha_n(y)\}|
+
\tau |\{i:\alpha_i(y)=\alpha_n(y)\}|
}{n},
\]

where \(\tau \sim \text{Unif}(0,1)\) is an independent random variable.

The corresponding prediction set is defined as
\[
\Gamma_\epsilon(x_n)=\{y:p_n^{\text{sm}}(y)>\epsilon\}.
\]

Under exchangeability, the smoothed conformal predictor achieves exact
finite-sample marginal validity, whereas the deterministic conformal
predictor is slightly conservative. \citep{vovk2022algorithmic}.
Operationally, this means that over repeated future predictions, the long-run miscoverage is controlled at level $\epsilon$ without requiring a correctly specified data model.

For practical classification and regression, \citet{vovk2022algorithmic} also emphasizes inductive (split) conformal prediction.
With proper training set $D_{\mathrm{tr}}$ and calibration set $D_{\mathrm{ca}}=\{z_i\}_{i=1}^m$, define calibration scores
\[
\alpha_i=A(D_{\mathrm{tr}},z_i),\quad i=1,\ldots,m,
\]
and candidate score $\alpha(x,y)=A(D_{\mathrm{tr}},(x,y))$.
Then
\[
p(x,y)=\frac{\left|\{i:\alpha_i\ge \alpha(x,y)\}\right|+1}{m+1},\qquad
\Gamma_\epsilon(x)=\{y:\;p(x,y)>\epsilon\}.
\]
For regression, a standard choice is residual nonconformity $A(D_{\mathrm{tr}},(x,y))=|y-\widehat y(x)|$.
For classification, label scores can be converted into nonconformity scores via monotone transformations, and the same p-value rule yields set-valued label predictions.
Compared with full conformal prediction, split conformal methods trade some statistical efficiency for large computational gains, which is one reason they are common in modern applications.
In general, the validity of conformal prediction does not depend on the choice of the nonconformity measure $A$, provided the examples are exchangeable. 

In this thesis, conformal calibration will be applied to survival outcomes under censoring. The following sections will review the background of survival analysis and existing work on conformal prediction for survival data.
...
A natural target is then the truncated event time
\[
T^*=\min(T,\tau),
\]
...
From an inferential perspective, finite windows make two issues central:
(1) calibration under censoring and
(2) clear interpretation of prediction targets.
...
The current thesis builds on the recent development of conformal methods for censored survival outcomes while focusing on a setting with a finite observation window. The two existing approaches reviewed above adopt substantially different strategies for handling censoring within conformal prediction.

\textcolor{revisionblue}{Relative to \citet{candes2023conformalized}, which constructs a one-sided lower predictive bound by conditioning on subjects with sufficiently large censoring times and correcting the induced covariate shift through weighting, the focus of this thesis is different. Instead of producing lower bounds for the untruncated survival time $T$, the target here is the truncated outcome $T^*=\min(T,\tau)$ under a finite follow-up window, and the goal is to construct predictive intervals for this windowed target with upper endpoint fixed at $\tau$.}

Conceptually, the methodological framework in this thesis is closer to the resampling-based conformal approach of \citet{qin2025conformal}. Their method reconstructs the joint distribution of $(T,X)$ using IPCW and generates calibration support through a bootstrap-based resampling scheme combined with working-model pivots. The implementation adopted here follows the same general philosophy of distribution recovery and pivot-based conformal calibration. The incremental contribution of the work includes (1) emphasizing a streamlined computational workflow by replacing the bootstrap layer used to generate calibration support with a direct Monte Carlo sampling step from the estimated joint distribution, which simplifies implementation while retaining the key conformal-style calibration mechanism. (2) extending the methodology to a finite observational window setting that considers administrative censoring due to practical factors (e.g. budget limits), which is more compatible with real-world applications. (3) Exploring and comparing more about the relationship between correctness of model assumptions and efficiency of conformal predictive intervals by conducting simulation studies, which provides empirical guidance on choosing working models given data. 
...
\textcolor{revisionblue}{To take account into the uncertainty at time $\tau$, our inferential target is a $(1-\alpha)$ predictive interval for $T^*$ given covariates with upper endpoint fixed at the finite horizon: for a new subject with covariate vector $z_0$, estimate}
\[
{\color{revisionblue}\widehat C_{1-\alpha}(z_0)=\big[\widehat L_\alpha(z_0),\tau\big]}
\]
...
\section{\texorpdfstring{\textcolor{revisionblue}{Working Model and Conformal Pivot}}{Working Model and Conformal Pivot}}
{\color{revisionblue}
...
\]
\end{enumerate}
This construction allocates the full tail error to the lower side and respects the
mixed distribution of $T^*$ at the administrative boundary.
}
...
subset of test observations, ordered by estimated risk score $x^\top\widehat\beta$,
...
\midrule
Weibull Plug-in $(U=\tau)$ & Correctly specified parametric working model with interval $[\widehat Q_{m,T^*}(\alpha\mid z),\tau]$ & Fitted by \texttt{survival::survreg(..., dist = "weibull")}; the fitted Weibull survival curve determines $\widehat F_{m,T^*}$ and $\widehat Q_{m,T^*}$. \\
Cox Plug-in $(U=\tau)$ & Correctly specified semi-parametric working model with interval $[\widehat Q_{m,T^*}(\alpha\mid z),\tau]$ & Fitted by \texttt{coxph}; $\widehat S_m(t\mid z)=\exp\{-\widehat H_0(t)\exp(z^\top\widehat\beta)\}$ with Breslow baseline hazard estimate. \\
Log-Normal Plug-in $(U=\tau)$ & Misspecified parametric working model with interval $[\widehat Q_{m,T^*}(\alpha\mid z),\tau]$ & Fitted by \texttt{survival::survreg(..., dist = "lognormal")}; this model is not compatible with the Weibull proportional hazards DGP. \\
...
\midrule
0.70 & Weibull Plug-in $(U=\tau)$ & 0.903 & [0.897, 0.910] & 1.040 & [1.028, 1.053] \\
0.70 & Cox Plug-in $(U=\tau)$ & 0.903 & [0.896, 0.910] & 1.040 & [1.028, 1.053] \\
0.70 & Log-Normal Plug-in $(U=\tau)$ & 0.911 & [0.905, 0.917] & 1.049 & [1.037, 1.061] \\
...
\midrule
0.80 & Weibull Plug-in $(U=\tau)$ & 0.900 & [0.895, 0.905] & 1.424 & [1.408, 1.439] \\
0.80 & Cox Plug-in $(U=\tau)$ & 0.901 & [0.895, 0.907] & 1.424 & [1.408, 1.440] \\
0.80 & Log-Normal Plug-in $(U=\tau)$ & 0.907 & [0.902, 0.912] & 1.430 & [1.414, 1.446] \\
...
\midrule
0.90 & Weibull Plug-in $(U=\tau)$ & 0.898 & [0.892, 0.904] & 2.143 & [2.120, 2.165] \\
0.90 & Cox Plug-in $(U=\tau)$ & 0.897 & [0.891, 0.902] & 2.142 & [2.120, 2.165] \\
0.90 & Log-Normal Plug-in $(U=\tau)$ & 0.903 & [0.896, 0.909] & 2.148 & [2.126, 2.170] \\
...
\midrule
0.99 & Weibull Plug-in $(U=\tau)$ & 0.897 & [0.891, 0.902] & 4.744 & [4.613, 4.874] \\
0.99 & Cox Plug-in $(U=\tau)$ & 0.895 & [0.889, 0.901] & 4.742 & [4.613, 4.872] \\
0.99 & Log-Normal Plug-in $(U=\tau)$ & 0.897 & [0.892, 0.903] & 4.745 & [4.614, 4.875] \\
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.94\textwidth]{Figures/model_comparison_uppertau_interval_examples.pdf}
\caption[Direct interval comparison in expanded baseline comparison]{Direct interval comparison at the representative middle horizon setting $\tau_q=0.90$. The same sampled test units are shown for all six fitted methods, ordered by the true linear predictor $x^\top\beta$.}
\label{fig:expanded_baseline_direct}
\end{figure}

Table~\ref{tab:expanded_baseline_summary} together with Figures~\ref{fig:expanded_baseline_tradeoff} and \ref{fig:expanded_baseline_direct} show that for correct specified models (Weibull and Cox), there's no difference between using and not using the conformal calibration. For the correctly specified Weibull model, the plug-in and conformal coverages are both $0.903$ at $\tau_q=0.70$, both $0.900$ at $\tau_q=0.80$, and both $0.897$ at $\tau_q=0.99$ up to the third decimal. The same pattern appears for the Cox working model, where the plug-in and conformal coverages differ by at most $0.002$ over the four horizons. 

In contrast, for the misspecified model (Log-normal), the parametric predictions return worse coverage, while the conformal calibration procedure maintain near-nominal coverage even though the working model is incorrect. At $\tau_q=0.70$, the log-normal plug-in coverage is $0.911$ and CP + Log-Normal is $0.904$; at $\tau_q=0.80$, the corresponding values are $0.907$ and $0.901$. These differences shrink as $\tau$ increases, and by $\tau_q=0.99$ both methods are essentially aligned at $0.897$ coverage with mean length about $4.74$.

Interval length behaves in the a similar way across all six methods. Averaging over the four $\tau$ regimes, mean interval length is $2.338$ for the Weibull plug-in and $2.337$ for CP + Weibull; the corresponding Cox values are both $2.337$. At the widest horizon $\tau_q=0.99$, all four correctly specified procedures have mean lengths essentially equal to $4.74$.

Overall, all six procedures target the same interval form $[\text{lower}(z),\tau]$, and most of them return very accurate coverage. The misspecified log-normal method showing conservativeness at smaller horizons, but get better when paired with the conformal calibration procedure.
...
\textcolor{revisionblue}{Third, in the expanded baseline working-model comparison, correct models (either with conformal calibration or not) perform well across all settings. Misspecified models perform worse without conformal calibration.}

Several limitations remain.
Current theory and experiments focus on independent censoring structures and limited working model class.
The simulations are extensive but still finite, and further stress tests under stronger covariate shifts, heavier censoring, and higher-dimensional regimes are warranted.
